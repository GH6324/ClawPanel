...
> [!NOTE]
> **v5.2.6 更新** — Telegram 通道现已按 OpenClaw schema 正确保存 `botToken`、自动补齐 `open` 模式所需白名单、保存后自动重载网关；同时新增 OpenClaw / 网关异常醒目提示，避免面板误显示“系统运行正常”。详见 [changelogs/v5.2.6.md](changelogs/v5.2.6.md)
