...
> [!NOTE]
> **最新发布** — `ClawPanel Pro v5.4.1` 与 `ClawPanel Lite v0.3.1` 已发布；这一轮补齐了工作流可视化画布编辑、`image_generate` 节点配置、微信会话对工作流的正确接管，以及图片/文件产物回传原会话的链路；同时清理了微信会话历史中的附件与系统噪音，并把仪表盘/活动日志时间统一到东八区展示。详情见 [changelogs/v5.4.1.md](changelogs/v5.4.1.md) 与 [changelogs/lite-v0.3.1.md](changelogs/lite-v0.3.1.md)。
