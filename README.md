...
> [!NOTE]
> **最新发布** — `ClawPanel Pro v5.4.0` 与 `ClawPanel Lite v0.3.0` 已发布；这一轮重点把面板整体同步到 OpenClaw `2026.4.8` 语义，新增 OpenClaw 原生任务板、Dashboard 任务压力摘要、统一 pairing 审批入口、扩展后的内置通道目录与动态 schema 渲染，同时重新整理了系统配置页的交互布局。Lite 内置 OpenClaw runtime 也同步提升到 `2026.4.8`。详情见 [changelogs/v5.4.0.md](changelogs/v5.4.0.md) 与 [changelogs/lite-v0.3.0.md](changelogs/lite-v0.3.0.md)。
