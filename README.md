...
[![Pro](https://img.shields.io/badge/Pro-5.3.3-violet?style=flat-square)](https://github.com/zhaoxinyi02/ClawPanel/releases)
[![Lite](https://img.shields.io/badge/Lite-0.2.3-2563eb?style=flat-square)](https://github.com/zhaoxinyi02/ClawPanel/releases)
...
> [!NOTE]
> **最新发布** — `ClawPanel Pro v5.3.3` 与 `ClawPanel Lite v0.2.3` 已发布；本次重点修复官方插件通道在 ClawHub 限流下的安装链路，并为企业微信（智能机器人）补齐 URL 回调 / 长连接双模式同页切换。若一键安装/手动更新没有拿到最新版，请优先重新拉取安装脚本或检查加速服务器元数据是否已同步。
...
### 面板聊天（v5.3.2 重点更新）
- 直接在面板里和本地 OpenClaw 对话，继续使用工作区、技能和本地文件上下文，不必再切回原生命令行
- 支持单 AI 对话与多 AI 顺序群聊，新建会话时可选择参与 Agent 与总结 Agent，适合代码协作、任务拆解和方案评审
- 支持会话列表预览、重命名、删除、代码块复制、中途停止与失败后恢复输入，适合作为日常调试和本地开发工作台
...
- **官方通道直装**：QQ 官方机器人、飞书、钉钉、企业微信等官方插件通道优先直接执行 `openclaw plugins install`，失败再回退 `npm install -g`
- **限流自动绕过**：若官方源因 `ClawHub 429` 限流失败，面板会自动先 `npm pack` 到本地 tarball，再交给 OpenClaw 安装
- **多来源安装**：第三方插件仍支持从 GitHub / 本地目录安装，适合自定义插件和私有扩展
...
#### [v0.2.3](changelogs/lite-v0.2.3.md) — 企业微信双模式与官方插件安装热修
- **🔧 企业微信智能机器人双模式**：同页支持 URL 回调与长连接切换，保存时自动规范化企业微信配置
- **📦 官方插件安装抗限流**：`openclaw plugins install` 遇到 `ClawHub 429` 时自动回退本地 tarball 安装
- **🧭 企业微信插件 ID 兼容**：兼容 `wecom-openclaw-plugin` 与历史 `wecom` entry，减少旧配置残留

#### [v0.2.2](changelogs/lite-v0.2.2.md) — 面板聊天与官方插件通道直装
- **💬 面板聊天上线**：Lite 可直接在面板内与内置 OpenClaw 对话，支持单 AI / 多 AI 顺序群聊与总结 Agent
- **📦 官方通道安装收口**：QQ 官方机器人、飞书、钉钉、企业微信等官方插件通道优先走 `openclaw plugins install`，失败后再回退 npm
- **🧾 版本文档同步**：更新最近版本展示、更新元数据与升级说明

#### [v0.2.1](changelogs/lite-v0.2.1.md) — 更新元数据与覆盖安装修复
- **🔄 更新元数据修复**：修复 Lite 更新源仍指向旧版本的问题
- **🔐 覆盖安装权限修复**：避免覆盖安装时把整个安装目录重新改成 root 所有
- **📻 Workflow / NapCat 体验收口**：继续减少回写刷屏和登录误判

<details>
<summary><b>更早 Lite 版本</b></summary>

#### [v0.2.0](changelogs/lite-v0.2.0.md) — 登录页与插件通道体验升级
- **🎨 界面统一**：同步新版登录页视觉风格，统一 Lite 与主面板的品牌展示
- **🔌 插件通道体验改善**：优先尝试官方包安装并增强网络异常下的恢复能力
- **🧠 面板聊天运行时修复**：修复错误继承插件 / 通道配置导致的运行时异常
...
#### [v5.3.3](changelogs/v5.3.3.md) — 企业微信双模式与官方插件安装热修
- **🔧 企业微信智能机器人双模式**：同页支持 URL 回调与长连接切换，保存时自动规范化企业微信配置
- **📦 官方插件安装抗限流**：`openclaw plugins install` 遇到 `ClawHub 429` 时自动回退本地 tarball 安装
- **🧭 企业微信插件 ID 兼容**：兼容 `wecom-openclaw-plugin` 与历史 `wecom` entry，减少旧配置残留与状态误判

#### [v5.3.2](changelogs/v5.3.2.md) — 面板聊天与官方插件通道直装
- **💬 面板聊天上线**：可直接在面板内与本地 OpenClaw 对话，支持单 AI / 多 AI 顺序群聊、总结 Agent 与代码块复制
- **📦 官方通道安装收口**：QQ 官方机器人、飞书、钉钉、企业微信等官方插件通道优先走 `openclaw plugins install`，失败后再回退 npm
- **🧾 版本与文档整理**：更新 README 最近版本展示、升级日志与更新元数据

#### [v5.3.1](changelogs/v5.3.1.md) — 更新元数据与外部托管修复
- **🔄 更新元数据修复**：修复面板与命令行仍停留旧版本号的问题
- **🌐 外部托管探测放宽**：减少 1Panel / Docker 外部托管 OpenClaw 被误报离线
- **📻 Workflow / NapCat 体验收口**：继续减少回写噪声和登录误判

<details>
<summary><b>更早 Pro 版本</b></summary>

#### [v5.3.0](changelogs/v5.3.0.md) — 登录页统一与插件通道安装补强
- **🎨 登录与界面体验统一**：重做透明 Logo、玻璃卡片与品牌比例，统一登录页与主面板视觉语言
- **🔌 插件通道安装补强**：修复插件安装后 OpenClaw 未自动重启、QQ 官方机器人等通道安装链路不稳的问题
- **📊 可观测性收口**：减少最近活动吞消息、审批噪声和重复离线提示
...
- 感谢所有为 `ClawPanel` 提交代码、反馈问题、参与设计讨论和帮助验证发布链路的开发者与社区贡献者
- 完整贡献者名单与贡献图谱请见：<https://github.com/zhaoxinyi02/ClawPanel/graphs/contributors>
...
### 企业微信（智能机器人）双模式（v5.3.3 热修）
- 同一页面支持 `使用 URL 回调` 与 `使用长连接` 两种接入方式，不再拆分页面
- URL 回调模式填写 `Token / EncodingAESKey / Webhook Path`
- 长连接模式填写 `Bot ID / Secret`，保存时自动规范化到 `channels.wecom`
- 兼容 `wecom-openclaw-plugin` 与历史 `wecom` 插件 ID，减少“已安装但面板识别不到”与错误写入旧 entry 的问题
